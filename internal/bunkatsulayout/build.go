...
func (b *builder) propertyBlock(p touki.Property) {
	...
	baseX := st.Margin + st.Indent
	valX := baseX + 5*st.em() + 4
	subX := baseX + 1.5*st.em()
	subValX := subX + 7*st.em()
	for _, r := range rows {
		switch {
		case r.Header:
			b.put(baseX, b.y, st.BodyPt, r.Label, scene.AlignLeft)
		case r.Sub:
			b.put(subX, b.y, st.BodyPt, justify(r.Label, 6), scene.AlignLeft)
			b.put(subValX, b.y, st.BodyPt, r.Value, scene.AlignLeft)
		default:
			b.put(baseX, b.y, st.BodyPt, justify(r.Label, 5), scene.AlignLeft)
			b.put(valX, b.y, st.BodyPt, r.Value, scene.AlignLeft)
		}
		b.y += st.LineH
	}
	b.y += 2
